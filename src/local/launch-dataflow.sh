...
if [ "$SKIPPER_CLI" == "true" ]; then
    ...
    java -jar $SCDIR/../../../spring-cloud-skipper/spring-cloud-skipper-server/target/spring-cloud-skipper-server-2.9.0-SNAPSHOT.jar \
...
if [ "$USE_PRO" == "true" ]; then
    ...
    java -jar $SCDIR/../../spring-cloud-dataflow-server/target/spring-cloud-dataflow-server-2.10.0-SNAPSHOT.jar \
        --spring.datasource.url='jdbc:mariadb://localhost:3306/dataflow' \
        --spring.datasource.username=spring \
        --spring.datasource.password=spring \
        --spring.datasource.driver-class-name=org.mariadb.jdbc.Driver
