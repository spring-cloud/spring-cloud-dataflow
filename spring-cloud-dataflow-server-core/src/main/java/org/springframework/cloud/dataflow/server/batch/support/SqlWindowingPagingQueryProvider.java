...
public class SqlWindowingPagingQueryProvider extends AbstractSqlPagingQueryProvider {
	...
	public String generateFirstPageQuery(int pageSize) {
		return "SELECT * FROM ( " +
			"SELECT " + (StringUtils.hasText(getOrderedQueryAlias()) ? getOrderedQueryAlias() + ".*, " : "*, ") +
			"ROW_NUMBER() OVER (" + getOverClause() +
			") AS ROW_NUMBER" +
			getOverSubstituteClauseStart() +
			" FROM " + getFromClause() +
			(!StringUtils.hasText(getWhereClause()) ? "" : " WHERE " + getWhereClause()) +
			(!StringUtils.hasText(getGroupClause()) ? "" : " GROUP BY " + getGroupClause()) +
			getOverSubstituteClauseEnd() +
			") " + getSubQueryAlias() + "WHERE " + extractTableAlias() +
			"ROW_NUMBER <= " + pageSize +
			" ORDER BY " + SqlPagingQueryUtils.buildSortClause(this);
	}
