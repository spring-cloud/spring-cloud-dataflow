...
public class SimpleJobService implements JobService, DisposableBean {
	 ...
	 */
	@Override
	public JobExecution restart(Long jobExecutionId) throws NoSuchJobException {
	...
	private Collection<String> getJsrJobNames() {
		Set<String> jsr352JobNames = new HashSet<>();
	...
	public int countJobs() {
		Collection<String> names = new HashSet<>(jobInstanceDao.getJobNames());
		return names.size();
	}
	...
	@Override
	public Collection<JobExecutionWithStepCount> listJobExecutionsForJobWithStepCount(
			String jobName, int start, int count) throws NoSuchJobException {
		checkJobExists(jobName);
