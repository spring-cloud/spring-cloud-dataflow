...
public class StreamDeploymentControllerTests {
	...
	private UpdatableStreamDeploymentController controller;

	@Mock
	private StreamDefinitionRepository streamDefinitionRepository;

	@Mock
	private UpdatableStreamService updatableStreamService;
	...
	public void setup() {
		...
		this.controller = new UpdatableStreamDeploymentController(streamDefinitionRepository, updatableStreamService);
	}
	...
	public void testDeployViaStreamService() {
		...
		verify(updatableStreamService).deployStream(argumentCaptor1.capture(), argumentCaptor2.capture());
	...
	public void testRollbackViaStreamService() {
		...
		verify(updatableStreamService).rollbackStream(argumentCaptor1.capture(), argumentCaptor2.capture());
	...
	public void tesPlatformsListViaSkipperClient() {
		when(updatableStreamService.platformList()).thenReturn(Arrays.asList(deployer));
		this.controller.platformList();
		verify(updatableStreamService, times(1)).platformList();
	}
	...
	public void testShowStreamInfo() {
		...
		when(this.updatableStreamService.info(streamDefinition.getName())).thenReturn(streamDeployment);
		when(this.updatableStreamService.state(anyListOf(StreamDefinition.class))).thenReturn(streamDeploymentStates);
