...
public class AboutControllerTests {
	...
	public void testListApplications() throws Exception {
		ResultActions result = mockMvc.perform(get("/about").accept(MediaType.APPLICATION_JSON)).andDo(print()).andExpect(status().isOk());
				...
				.andExpect(jsonPath("$.versionInfo.implementation.version", is("1.2.3.RELEASE")))
				.andExpect(jsonPath("$.versionInfo.implementation.checksumSha1", is("2dd0de58ebd59ac0d74106971dbc2fd03bec1bcd")))
				.andExpect(jsonPath("$.versionInfo.implementation.checksumSha256").doesNotExist())
				.andExpect(jsonPath("$.versionInfo.core.name", is("Spring Cloud Data Flow Core")))
				.andExpect(jsonPath("$.versionInfo.core.checksumSha1").doesNotExist())
				.andExpect(jsonPath("$.versionInfo.core.checksumSha256").doesNotExist())
				.andExpect(jsonPath("$.versionInfo.dashboard.name", is("Spring Cloud Dataflow UI")))
				.andExpect(jsonPath("$.versionInfo.dashboard.checksumSha1").doesNotExist())
