...
```
java -jar spring-cloud-skipper-shell-{project-version}.jar --spring.cloud.skipper.client.uri=http://localhost:9123/api
```
...
[source,bash,subs=attributes]
```
$ java -jar spring-cloud-skipper-shell-{project-version}.jar --spring.cloud.skipper.client.uri=http://localhost:9123/api search
```
...
spec:
  ...
  resourceMetadata:  maven://org.mysample:mypackage:jar:metadata:{{version}}
  version: {{version}}
