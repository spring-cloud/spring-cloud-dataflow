...
public class DataFlowClientAutoConfiguration {
	...
	public DataFlowOperations dataFlowOperations() throws Exception {
		...
		final HttpClientConfigurer httpClientConfigurer = HttpClientConfigurer
				.create(new URI(properties.getServerUri()))
		...
		if (StringUtils.hasText(this.properties.getAuthentication().getAccessToken())) {
			template.getInterceptors().add(new OAuth2AccessTokenProvidingClientHttpRequestInterceptor(
					this.properties.getAuthentication().getAccessToken()));
			...
			template.getInterceptors()
					.add(new OAuth2AccessTokenProvidingClientHttpRequestInterceptor(accessTokenValue));
	...
	private OAuth2AuthorizedClientService shellAuthorizedClientService(
	...
	private ClientHttpRequestInterceptor bearerTokenResolvingInterceptor(
		...
		OAuth2AuthorizedClientService shellAuthorizedClientService = shellAuthorizedClientService(
