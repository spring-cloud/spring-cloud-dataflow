...
public class TaskTemplate implements TaskOperations {
  ...
  TaskTemplate(RestTemplate restTemplate, RepresentationModel<?> resources,
      String dataFlowServerVersion) {
    ...
    Assert.notNull(resources.getLink(EXECUTION_RELATION_BY_NAME),
        "Execution by name relation is required");
    ...
    if (VersionUtils.isDataFlowServerVersionGreaterThanOrEqualToRequiredVersion(
        VersionUtils.getThreePartVersion(dataFlowServerVersion), VALIDATION_RELATION_VERSION)) {
      Assert.notNull(resources.getLink(VALIDATION_REL),
          "Validiation relation for tasks is required");
    }

    if (VersionUtils.isDataFlowServerVersionGreaterThanOrEqualToRequiredVersion(
        VersionUtils.getThreePartVersion(dataFlowServerVersion),
        EXECUTIONS_CURRENT_RELATION_VERSION)) {
      Assert.notNull(resources.getLink(EXECUTIONS_CURRENT_RELATION),
          "Executions current relation is required");
    }
  ...
  public long launch(String name, Map<String, String> properties, List<String> arguments,
      String alternateComposedTaskRunnerApp) {
    ...
    return restTemplate.postForObject(executionByNameLink.expand(name).getHref(), values,
        Long.class, name);
  }
  ...
  public void destroy(String name, boolean cleanup) {
    String url = (cleanup) ? definitionLink.expand(name).getHref() + "?cleanup=true"
        : definitionLink.expand(name).getHref();
  ...
  public TaskAppStatusResource validateTaskDefinition(String taskDefinitionName)
    ...
    if (validationLink == null) {
      throw new OperationNotSupportedException(
          "Task Validation not supported on Data Flow Server version " + dataFlowServerVersion);
    }
