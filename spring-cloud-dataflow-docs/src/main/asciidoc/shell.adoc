...
* The special characters used in property files (both Java and YAML) needs to be escaped. For example `\` should be replaced  by `\\`, '\t` by `\\t` and so fort.
* For Java property files (`--propertiesFile` <FILE_PATH.properties>) the property values should not be surrounded by quotes! Even if they contain spaces. E.g.
----
filter.expression=payload > 5
----
* For YAML property files (`--propertiesFile` <FILE_PATH.yaml>) though the values need to be surrounded in double quotes.
----
app:
    filter:
        filter:
