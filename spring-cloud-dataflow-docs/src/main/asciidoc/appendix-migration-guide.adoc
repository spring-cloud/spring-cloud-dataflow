...
* Alternatively, if you’d like to patch any of the out-of-the-box stream/task applications, you can 
...
* counter-sink:
** The peripheral `redis` is not required in Spring Cloud Data Flow. If you intend to use the `counter-sink`, `redis` then becomes required, and you’re expected to have your own running `redis` cluster
* field-value-counter-sink:
** The peripheral `redis` is not required in Spring Cloud Data Flow. If you intend to use the `field-value-counter-sink`, `redis` then becomes required, and you’re expected to have your own running `redis` cluster
* aggregate-counter-sink:
** The peripheral `redis` is not required in Spring Cloud Data Flow. If you intend to use the `aggregate-counter-sink`, `redis` then becomes required, and you’re expected to have your own running `redis` cluste
...
production-ready binders and are available as GA releases. We also have an experimental version of link:https://github.com/spring-cloud/spring-cloud-stream-binder-gemfire[Gemfire] binder.
...
* Unlike Spring XD, these “Tasks” doens't require explicit deployment; instead, a task is ready to be 
launched directly
...
|job launch        |task list
|job list        |task launch
