...
Properties to be applied for all deployed Tasks are defined in the `src/kubernetes/server/server-config-(binder).yaml` file and for Streams in `src/kubernetes/skipper/skipper-config-(binder).yaml`. Replace `(binder)` with the messaging middleware you are using, for example `rabbit` or `kafka`. By default properties are defined in the `application.yaml` configuration block under the namespace `spring.cloud.dataflow.task.platform.kubernetes.accounts.default` for Tasks and `spring.cloud.skipper.server.platform.kubernetes.accounts.default` for Streams.

==== Memory and CPU Settings

Applications are deployed with default memory/cpu settings. You might find these settings may need to be adjusted which can be done as the following example shows (replace `<app>` with the name of the app for which you want to set the memory):
...
You can also control the default values to which to set the `cpu` and `memory` requirements for the pods that are created as part of application deployments, for example:
...
The same can declared as part of the global configuration for Streams:
