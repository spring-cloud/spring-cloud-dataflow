...
* `shell`: Passes all application properties and command line arguments as environment variables. Each of the application/commandline argument properties is transformed into an uppercase string and `.` characters are replaced with `_`.
...
Secrets can be referenced and their entire data contents decoded and inserted into the pod(s) environment as individual variables.
Refer to the https://kubernetes.io/docs/tasks/inject-data-application/distribute-credentials-secure/#configure-all-key-value-pairs-in-a-secret-as-container-environment-variables[Configure all key-value pairs in a Secret as container environment variables] section of the Kubernetes reference for more information.
...
[source,options=nowrap]
----
deployer.<app>.kubernetes.secretRefs=testsecret
----
====

Multiple secrets can also be specified, for example:

====
[source,options=nowrap]
----
deployer.<app>.kubernetes.secretRefs=[testsecret,anothersecret]
----
====

Replace `<app>` with the name of your application and the `secretRefs` attribute with the appropriate values for your application environment and secret.
...
Replacing the items of `secretRefs` with one or more secret names.
...
ConfigMap's can be referenced and their entire data contents decoded and inserted into the pod(s) environment as individual variables.
Refer to the https://kubernetes.io/docs/tasks/configure-pod-container/configure-pod-configmap/#configure-all-key-value-pairs-in-a-configmap-as-container-environment-variables[Configure all key-value pairs in a ConfigMap as container environment variables] section of the Kubernetes reference for more information.
...
[source,options=nowrap]
----
deployer.<app>.kubernetes.configMapRefs=testcm
----
====

Multiple ConfigMap's can also be specified, for example:

====
[source,options=nowrap]
----
deployer.<app>.kubernetes.configMapRefs=[testcm,anothercm]
----
====

Replace `<app>` with the name of your application and the `configMapRefs` attribute with the appropriate values for your application environment and ConfigMap.
...
Replacing the items of `configMapRefs` with one or more secret names.
...
By default the image used is `busybox`, which can be customized if needed.
...
Replace `<app>` with the name of your application and the `statefulSetInitContainerImageName` attribute with the appropriate value for your environment.
