...
The collected metrics and monitoring data are then viewable via Grafana dashboards. Out of the box, Grafana comes pre-configured with a Prometheus data source connection along with dashboards for streams and applications.
+
Run the following commands to deploy create the cluster role, binding and service account:
+
...
+
IMPORTANT: If you are deploying to Minikube that does not provide a load balancer, you can obtain the dashboard address by running: `minikube service --url grafana`.
+
