...
For streaming pipelines, we also need the link:https://cloud.spring.io/spring-cloud-skipper/[Skipper] server (for lifecycle management) and a messaging middleware option, such as Apache Kafka or RabbitMQ.
...
See https://kubernetes.io/docs/user-guide/prereqs/[Installing and Setting up kubectl] for installation instructions.
...
Data Flow delegates the streams lifecycle management to Skipper. You need to deploy link:https://cloud.spring.io/spring-cloud-skipper/[Skipper] to enable the stream management features.
...
We also use the https://github.com/spring-cloud/spring-cloud-kubernetes[Spring Cloud Kubernetes library] to access the Kubernetes https://kubernetes.io/docs/user-guide/configmap/[`ConfigMap`] and https://kubernetes.io/docs/user-guide/secrets/[`Secrets`] settings.
...
NOTE: If you use Minikube, you must include the port -- for example: `dataflow config server --uri https://192.168.99.100:31991`
...
* RabbitMQ: `dataflow:>app import --uri https://bit.ly/Einstein-SR2-stream-applications-rabbit-docker`
* Kafka: `dataflow:>app import --uri https://bit.ly/Einstein-SR2-stream-applications-kafka-docker`
