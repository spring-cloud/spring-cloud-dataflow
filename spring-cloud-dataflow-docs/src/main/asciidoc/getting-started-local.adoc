...
dataflow:>app register --name log --type sink --uri maven://org.springframework.cloud.stream.app:log-sink-rabbit:2.0.0.RELEASE
...
----
$ jps | grep rabbit
12643 log-sink-rabbit-2.0.0.RELEASE.jar
12645 http-source-rabbit-2.1.0.RELEASE.jar
----

[[getting-started-local-spring-cloud-dataflow-streams-upgrading]]
==== Upgrading

Before we start upgrading the log-sink version to 2.1.0.RELEASE, we will have to register that version in the app registry.
...
By executing the Unix `jps` command, you can see the two java processes running, but now the log application is version 2.1.0.RELEASE, as shown in the following listing:

[source,bash]
----
$ jps | grep rabbit
22034 http-source-rabbit-2.1.0.RELEASE.jar
22031 log-sink-rabbit-2.1.0.RELEASE.jar
----

Now you can look in the log file of the Skipper server to find the location of the httptest-log app.
To view the log, use the following commands:

`cd` to the directory `/<location of log>/httptest.log-v2` and `tail -f stdout_0.log`
...
"spec":
  ...
  "resourceMetadata": "maven://org.springframework.cloud.stream.app:time-source-rabbit:jar:metadata:2.1.0.RELEASE"
...
"spec":
  ...
  "resourceMetadata": "maven://org.springframework.cloud.stream.app:log-sink-rabbit:jar:metadata:2.1.0.RELEASE"
  "version": "2.1.0.RELEASE"
  "applicationProperties":
    "spring.metrics.export.triggers.application.includes": "integration**"
...
By executing the Unix `jps` command, you can see the two java processes running, but now the log application is back to 2.0.0.RELEASE.
...
$ jps | grep rabbit
22034 http-source-rabbit-2.1.0.RELEASE.jar
23939 log-sink-rabbit-2.0.0.RELEASE.jar
----

Now you can look in the log file of the Skipper server to find the location of the httptest-log app.
To view the log, use the following commands:

`cd` to the directory `/<location of log>/httptest.log-v3` and `tail -f stdout_0.log`
