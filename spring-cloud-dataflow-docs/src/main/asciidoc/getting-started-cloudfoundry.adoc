...
====
+
You need to fill in `<org>`, `<space>`, `<email>`,  `<password>`, `<middlewareServiceName>` (Rabbit or Kafka) and `<services>` (such as PostgresSQL) before running these commands.
...
This section covers some things to be aware of when you install into a Cloud Foundry instance.
...
In the future, you will be able to deploy tasks to multiple platforms. However, for 2.0.0.M1, you can deploy to only a single platform and the name must be `default`.
...
The Spring Cloud Data Flow server does not have a default remote Maven repository configured.
...
When you create a stream, you msut use a unique name (one that might not be taken by another application on PCF/PWS).
...
You can see the year (`2019` at the time of this writing) printed in the logs. The execution status of the task is stored in the database, and you can retrieve information about the task execution by using the `task execution list` and `task execution status --id <ID_OF_TASK>` shell commands or though the Data Flow UI.
