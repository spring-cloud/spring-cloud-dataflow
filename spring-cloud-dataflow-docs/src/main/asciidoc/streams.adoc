...
of the upcoming `1.4.0` release are not explicitly supported, yet.

The cause is in the used `spring-boot-maven-plugin`. As a result you will observe
all available application properties being listed, not just the whitelisted ones. As a temporary
workaround, you can override the managed version of your app's `spring-boot-maven-plugin`
...
When you execution `stream deploy` shell command, you can optionally provide a
comma delimited list of key=value pairs known as `deployment properties`.
These properties allow you to specify deployment instruction for each application
in the stream.
