...
One can connect to a specific source destination name located in the broker (Rabbit, Kafka) and connect it to a downstream set of apps.
...
In the above stream, the destination name `mySource` is the `source` destination name and the stream receives messages from the source destination `mySource` at the broker and connects it to the `log` app.
...
In the above stream, both the source and sink destinations are located in the broker. The messages flow from the source destination to the sink destination via a `bridge` app that connects the source and sink destinations.
