...
Alternatively, you can use Data Flow for Kuberenetes to deploy a stream to a single namespace on a Kubernetes cluster.
See http://cloud.spring.io/spring-cloud-dataflow/#platform-implementations[the Spring Cloud Data Flow project page] for a list of implementations.
...
When deploying a stream in Data Flow using Skipper, you can specify which platform to use.
...
You create and deploy a stream by using Skipper in two steps: creating the stream definition and then deploying the stream.
...
// TODO We really shouldn't make promises. We might change this intention, but we've set the expectation.
...
=== Stream Manifests
Skipper keeps a "`manifest`" of the all the applications, their application properties, and deployment properties after all values have been substituted.
...
Future versions of Skipper will support rolling upgrades and other types of checks (for example, manual) to continue the upgrade process.
// TODO We shouldn't make promises, because plans change.
...
// TODO We should not make promises.
...
// TODO I'm not sure those three lines should be one example, but I couldn't see why it was two listings.
