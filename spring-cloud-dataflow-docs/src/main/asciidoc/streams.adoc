...
If you prefer to deploy streams programmatically, you can do so by using the fluent style DSL api provided by the `spring-cloud-dataflow-rest-client` module.
The java DSL is a convenient wrapper around the `DataFlowTemplate` class that makes it simpler to create and deploy streams.
...
In order to create streams you need to provide a `Stream.Builder` instance first.
[source,java]
----
@Bean
public Stream.Builder builder() throws Exception{
    return new Stream.Builder(new DataFlowTemplate(new URI("http://localhost:9393")));
}
----

=== Creating Streams using the Java DSL

The java DSL offers two styles to create Streams, if you prefer the pipes and filters DSL provided by dataflow you can use the `definition()` method and provide your own DSL definition.
If you prefer a more fluent style usage, you can daisy chain methods via the `source()`, `processor()`, `sink()` methods.
To demonstrate both styles we will create a simple stream using both approaches.

[source,java]
----
@Autowired
private Stream.Builder builder; #<1>

public void usingDefinition() throws Exception{
   ...
   while(!woodchuck.getStatus().equals("deployed")){ # <2>
      Thread.sleep(5000);
   }
...
public void fluent() throws Exception{
   ...
   Stream woodchuck = builder.name("woodchuck")
         ...
         .sink(new StreamApplication("log")
               .addDeploymentProperty("count", 2)) # <3>
         .create().deploy();
...
----
<1> Assumes that a bean of type `Stream.Builder` is available in your application context.
<2> You can poll for the stream status using `getStatus()` method
<3> When using the fluent style and adding deployment properties to the application you don't need to prefix it with `deployer.<app_name>`
[NOTE]
...
Attempting to create or deploy a stream that contains an unknown app will throw an exception.

The Stream applications can also be beans within your application that could be injected to create Streams.
...
@Bean
public StreamApplication httpApplication(){
    return new StreamApplication("http");
}

public Stream createStream(){
    return builder.name("sample")
...
----
<1> `source()`, `processor()` and `sink()` methods have an overloaded version that accepts just a String and creates a Stream application.
