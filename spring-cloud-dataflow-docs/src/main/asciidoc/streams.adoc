...
==== Passing application properties when creating stream
...
The shell command `app info` helps viewing the white-listed application properties for the application.
For more info on the property white listing refer <<spring-cloud-dataflow-stream-app-whitelisting>>

Below is the white listed properties for the app `time`:
...
Below is the white listed properties for the app `log`:
...
The application properties for `time` and `log` apps can be specified at the time of `stream` creation as follows:
...
==== Passing application properties when deploying stream

The application properties can also be specified when deploying the stream. When specified during deployment, these application properties can either be specified as
 `short-hand` property names (applicable for white-listed properties) or fully qualified property names. The application properties should have the prefix "app.<app name/label>".
...
can be deployed with application properties using the`short-hand` property names:
...
[source,bash]
----
stream deploy ticktock --properties "app.a.fixed-delay=4,app.b.log.level=ERROR"
----


==== Overriding application properties during stream deployment

Application properties that are defined during deployment overrides the same properties defined during the stream creation.
...
For instance, one can specify how many number of instances need to be deployed for the specific application defined in the stream using the deployment property called `count`.
The stream partitioning properties are also passed as `deployment` properties during stream deployment. These properties are optionally provided as a comma delimted list of key=value pairs.
...
Note that `count` is the *reserved* property name by the underlying deployer. Hence, if the application also has a custom property named `count`, it is *not* supported
 when specified during stream `deployment` as it conflicts with the `instance` count deployer property. Instead, the `count` as a custom application property can be
 specified at the stream `creation` when it will be considered as an app property.
...
content-based routing so that messages containing the identical data value(s) are
always routed to the same app instance. You can use the Deployment Properties to
declaratively configure a partitioning strategy to route each message to a specific
consumer instance.
