...
The java DSL is a convenient wrapper around the `DataFlowTemplate` class that makes it simpler to create and deploy applications.
...
To create a Stream you need to supply a `DataFlowTemplate` to the `Stream.Builder` static method:
[source,java]
----
DataFlowTemplate client = //...;
Stream ticktock = Stream.Builder(client).name("ticktock").definition("time | log").create();
----
[NOTE]
For your convenience, the `spring-cloud-dataflow-rest-client` library provides a bean of type `Stream.StreamNameBuilder` with the `DataFlowTemplate` already configured.
...
public void usingDefinition() throws Exception{
   ...
   while(!woodchuck.getStatus().equals("deployed")){ # <1>
      Thread.sleep(5000);
   }
...
public void fluent() throws Exception{
   ...
   Stream woodchuck = builder.name("woodchuck")
         ...
         .sink(new StreamApplication("log")
               .addDeploymentProperty("count", 2)) # <2>
         .create().deploy();
...
<1> You can poll for the stream status using `getStatus()` method
<2> When using the fluent style and adding deployment properties to the application you don't need to prefix it with `deployer.<app_name>`
