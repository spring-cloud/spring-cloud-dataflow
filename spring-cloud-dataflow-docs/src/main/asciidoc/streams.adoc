...
For example, if you've have to register `SNAPSHOT` release of Rabbit binder based `http` and `log` applications, 
...
where the keys are formatted as `<type>.<name>` and the values are the URIs. For example, if you've have to 
register `SNAPSHOT` release of Rabbit binder based `http` and `log` applications, 
you'd have the following in a property file [_eg: stream-apps.properties_] and import the apps in bulk.
