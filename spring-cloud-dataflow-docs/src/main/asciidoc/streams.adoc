...
This shorthand behavior is discussed more in the section on <<spring-cloud-dataflow-stream-app-allowlisting>>.
...
The allowlisting support works only for uber-jar application artifacts! At the moment, the metadata properties are not retrievable from the dockerized application images directly — a dedicated companion metadata JAR is required.

The `configuration-properties.names` can allowlist properties that are defined in the property class annotated with `@ConfigurationProperties`. The properties defined with `@Value` annotations cannot be allowlisted.
====
...
configuration properties metadata and the allowlisting file described in the previous section.
...
For supporting property allowlisting, Spring Cloud Stream applications running in Spring Cloud Data Flow may include the Spring Boot `configuration-processor` as an optional dependency, as shown in the following example:
...
The shell command `app info --name <appName> --type <appType>` displays the allowlisted application properties for the application.
For more info on the property allowlisting, refer to <<spring-cloud-dataflow-stream-app-allowlisting>>

The following listing shows the allowlisted properties for the `time` app:
...
The following listing shows the allowlisted properties for the `log` app:
...
These "'short-form'" property names are applicable only for the allowlisted properties. In all other cases, only fully qualified property names should be used.
