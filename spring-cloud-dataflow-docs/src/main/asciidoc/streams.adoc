...
It is useful to set the content-type to configure the type conversion between the apps inside the stream
definition. The supported property names are `inputType` and `outputType` to specify the content type
for the incoming data and outgoing data respectively.
...
and processes as Spring Tuple(hence the inputType for the filter app). The `transform` processes the Tuple
data and sends the processed data to the downstream `log` application.
...
Based on the amount of processing involved, the contentType can be specified either as the `--outputType` in the upstream app or
as the `--inputType` in the downstream app. For instance, in the above stream, instead of specifying the `--inputType`
at the 'transform' application, the option `--outputType=application/x-spring-tuple` can be specified at the 'http' application.
