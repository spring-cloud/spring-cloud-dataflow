...
allowing quicker feedback when using _e.g._ `app info`
...
3. finally, for environments that don't deal with boot uberjars directly (for example for Docker based runtimes), this
is the only way to provide metadata about the properties supported by the app.

Remember though, that this is entirely optional when dealing with uberjars. The uberjar itself is a source of metadata.
...
Here is an example for a Dockerized app:
