...
The application properties can also be specified when deploying the stream. When specified during deployment, these application properties require fully qualified
property names with "app.<app name/label>" as the prefix.
...
can be deployed with application properties as follows:

[source,bash]
----
dataflow:>stream deploy ticktock --properties "app.time.trigger.fixed-delay=4,app.log.log.level=ERROR"
----

Note the prefix `app.time` and `app.log` with the fully qualified properties (not the short-form properties we can define during stream creation)
 `trigger.fixed-delay` and `log.level` from the `app info` above.
...
[source,bash]
----
stream deploy ticktock --properties "app.a.trigger.fixed-delay=4,app.b.log.level=ERROR"
----
...
Note the `short-form` and `fully qualified form` of white listed application properties in stream creation and deployment respectively.
