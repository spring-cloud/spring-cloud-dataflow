...
Spring Cloud Data Flow `local` server is configured to use `RollingFileAppender` for logging and you should enable the spring profile named 'local'.
You need to pass the property `--spring.profiles.active=local` when starting the uber jar to trigger the configuration of logging in local mode.
The profile is needed since writing to a log file is not the recommended way to gather logs when running on Cloud Platforms such as Cloud Foundry and Kubernetes.
...
<<configuration-local-security-basic-authentication, Traditional Authentication>> option.
...
<<customizing-local-authorization, customizing authorization>> for more details.
====
...
[[configuration-local-security-role-mapping]]
====== Role Mappings
...
[[configuration-local-security-ldap-authentication]]
===== LDAP Authentication
...
[[configuration-local-security-authentication-via-shell]]
...
[[customizing-local-authorization]]
      ...
      security:
        authorization:
          ...
          rules:
            ...
            - GET    /runtime/stream                 => hasRole('ROLE_VIEW')
...
[[authorization-local-shell-and-dashboard]]
...
[[configuration-local-monitoring-management]]
...
[[configuration-local-monitoring-deployed-applications]]
...
[source,bash,subs=attributes]
----
$ java -jar spring-cloud-dataflow-server-{project-version}.jar --spring.cloud.dataflow.metrics.collector.uri=http://localhost:8080
----
...
The first is by exposing a `/runtime/stream` HTTP endpoint that acts as a proxy to the Metrics Collector endpoint.
