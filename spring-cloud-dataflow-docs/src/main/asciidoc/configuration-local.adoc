...
[source,bash,subs=attributes]
----
$ java -jar spring-cloud-dataflow-server-local-{project-version}.jar --spring.config.additional-location=/home/joe/maven.yml
----
...
For example, pass the property `--spring.profiles.active=local` when starting the uber jar.
