...
You can easily modify these pre-built application using a version of https://start-scs.cfapps.io/[Spring Intializr for Stream applications].
...
In this architecture, the Application DSL description that describes a group of related applications is POSTed to the Data Flow Server.
...
In this example, the `baristaApp` and sends messages on either the `hotDrinks` destination or the `coldDrinks` destination.
