...
The Data Flow server uses the link:https://github.com/pivotal-cf/java-cfenv[Java CFEnv] library to create the DataSource with a default connection pool size of 4.
...
=== Database Connection Pool
The Data Flow server uses the https://github.com/pivotal-cf/java-cfenv[Java CFEnv] library to create the DataSource with a default connection pool size of `4`.
...
image::images/cf-getting-started-security-no-roles.png[Dashboard without roles, scaledwidth="100%"]
