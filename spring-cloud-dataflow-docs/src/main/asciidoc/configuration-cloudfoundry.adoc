...
* Establish the URL to the PCF-Scheduler by setting the `SPRING_CLOUD_DATAFLOW_TASK_PLATFORM_CLOUDFOUNDRY_ACCOUNTS[default]_SCHEDULER_PROPERTIES_SCHEDULER_URL` environment variable.
...
- name: data-flow-server
  ...
  env:
    ...
    SPRING_CLOUD_SKIPPER_CLIENT_SERVER_URI: https://<skipper-host-name>/api
    SPRING_CLOUD_DATAFLOW_TASK_PLATFORM_CLOUDFOUNDRY_ACCOUNTS[default]_SCHEDULER_PROPERTIES_SCHEDULER_URL: https://scheduler.local.pcfdev.io
services:
- mysql
----

Where the `SPRING_CLOUD_DATAFLOW_TASK_PLATFORM_CLOUDFOUNDRY_ACCOUNTS[default]_SCHEDULER_PROPERTIES_SCHEDULER_URL` has the following format: `scheduler.<Domain-Name>` (for
