...
. To enable Spring Cloud Skipper along with Metrics support, the following modifications need to be made. In this example, its assumed the stock `docker-compose.yml` file is being used as a starting point. Customizations can still be applied, for example swapping Kafka with Rabbit by using the examples above.
+
...
----
+
NOTE: When using Skipper, launched applications and their logs reside on the `skipper` container rather than the `dataflow-server` container. For example, when viewing stream logs as described earlier, the `skipper` container would be referenced instead:
+
[source,yaml,subs=attributes]
----
