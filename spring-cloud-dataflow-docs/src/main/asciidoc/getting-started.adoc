...
and if you'd want to use any or all of the supported out-of-the-box applications, you have to pass 
Spring's maven repo via `--maven.remote-repositories.repo1.url=https://repo.spring.io/libs-snapshot` 
as the remote repository. Like in the above example, the remote repositories can be specified along with their authentication (if needed). If the remote repositories are behind a proxy, then the proxy 
properties can be specified as above.
+
NOTE: Spring Cloud Data Flow server implementations (local, cf, mesos, yarn, or kubernetes) do not have 
'any' default maven remote repository configured. This is intentionally designed to provide the flexibility for 
...
so if you want to use them, you 'must' set it as the remote repository as discussed above.
+
...
"remote-repositories": { "repo1": { "url": "https://repo.spring.io/libs-snapshot", "auth": { "username": "repo1user", "password": "repo1pass" } }, "repo2": { "url": "https://repo2" } }, "proxy": { "host": "proxyhost", "port": 9018, "auth": { "username": "proxyuser", "password": "proxypass" } } } }'
----
+
. Launch the shell:
+
...
[[getting-started-security-self-signed-certificates]]
...
[[getting-started-security-basic-authentication]]
=== Basic Authentication
...
[[getting-started-security-file-based-authentication]]
...
[[getting-started-security-ldap-authentication]]
==== LDAP Authentication
...
[[getting-started-security-oauth2]]
=== OAuth 2.0
