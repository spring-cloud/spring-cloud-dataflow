...
and `http://repo1.maven.org/maven2` is the default remote repository. This default remote repository will not be used if user provides explicit remote repositories.
Like in the above example, the remote repositories can be specified along with their authentication (if needed). If the remote repositories are behind a proxy,
