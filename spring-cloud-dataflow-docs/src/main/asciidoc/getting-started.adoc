...
When starting the server you can specify the option `serverStore` to choose `redis` or `db` based repositories for StreamDefinition, ModuleDeploymentId repositories.
By default, the `redis` based repositories are used.
