...
You need to have an RDBMS for storing stream, task, and app states in the database. By default, the `local` Data Flow server uses embedded H2 database for this.
...
+
There are two options for deploying Streams.  The "traditional" way that Data Flow has always used and a new way that delegates to the Skipper server.  Deploying by using Skipper enables you to update and rollback the streams while the traditional way does not.
+
. Create Streams without Skipper
+
You can now use the shell commands to list available applications (source/processors/sink) and create streams, as shown in the following example:
+
[source,bash]
----
...
----
+
NOTE: You need to wait a little while, until the apps are deployed successfully, before posting data.
