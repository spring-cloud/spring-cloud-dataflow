...
$ export SKIPPER_VERSION=1.1.2.RELEASE
$ docker-compose up
----
+
...
[source,bash,subs=attributes]
----
$ DATAFLOW_VERSION={local-server-image-tag} SKIPPER_VERSION=1.1.2.RELEASE docker-compose up
----
+
...
C:\ set SKIPPER_VERSION=1.1.2.RELEASE
C:\ docker-compose up
----
+
...
. [[getting-started-customizing-spring-cloud-dataflow-docker-mysql]]To use MySQL rather than the H2 embedded database, add the following configuration under the `services` section:
+
...
. [[getting-started-customizing-spring-cloud-dataflow-docker-redis]]To enable analytics using redis as a backend, add the following configuration under the `services` section:
+
[source,yaml,subs=attributes]
----
  redis:
...
. To run Spring Cloud Skipper along with Metrics support, the following modifications need to be made. In this example, its assumed the stock `docker-compose.yml` file is being used as a starting point. Customizations can still be applied, for example swapping Kafka with Rabbit by using the examples above.
+
First, after the `zookeeper` service definition:
+
* Add MySQL as described in <<getting-started-customizing-spring-cloud-dataflow-docker-mysql,Using MySQL>>
* Add Redis as described in <<getting-started-customizing-spring-cloud-dataflow-docker-redis,Enable analytics using Redis>>
+
Then add Skipper and Metrics under the `dataflow-server` service definition:
+
[source,yaml,subs=attributes]
----
  skipper-server:
...
----
+
Remove application port mapping from `dataflow-server` as they are now assigned to the `skipper-server` container:
+
[source,yaml,subs=attributes]
----
  dataflow-server:
...
----
+
And lastly append the following environment variables to the `dataflow-server` `environment` block:
+
[source,yaml,subs=attributes]
----
...
----
+
NOTE: With Skipper, launched applications and their logs reside on the `skipper` container rather than the `dataflow-server` container. For example, when viewing stream logs as described earlier, the `skipper` container would be referenced instead:
+
[source,yaml,subs=attributes]
----
...
----
+
Starting 2.0.x, the Data Flow Server run in `skipper` mode only when the Stream feature is enabled.
