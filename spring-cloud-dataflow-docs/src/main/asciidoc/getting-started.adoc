...
+
```
dataflow:>app register --name http --type source --uri maven://org.springframework.cloud.stream.app:http-source-rabbit:1.3.1.RELEASE
Successfully registered application 'source:http'

dataflow:>app register --name log --type sink --uri maven://org.springframework.cloud.stream.app:log-sink-rabbit:1.3.1.RELEASE
...
+
By default, the application registry is empty.
As an example, register one task applications, `timestamp`.
+
[source,bash]
----
...
Maven repository location or configure proxy settings.  See <<configuration-maven>> for more information.
+
In this getting started section, we show how to create a task definition and launch it.
+
...
+
Use the `task create` command to create a task definition using the `timestamp` application.
+
[source,bash]
----
