...
image::images/dataflow-security-default-user.png[Default Spring Boot user credentials , scaledwidth="100%"]

NOTE: Dedicated Basic Authentication support is available since Spring Cloud Data Flow 1.1.0. Also,
please be aware of inherent issues of Basic Authentication and _logging out_, since the
credentials are cached by the browser and simply browsing back to application pages will log you back in.
...
as described below. Each user must be assigned a password and a role:
...
<3> Each map `value` is made of a corresponding password and role

NOTE: File based authentication support is available since Spring Cloud Data Flow 1.1.0.
...
image::images/dataflow-security-github.png[Register an OAuth Application for GitHub , scaledwidth="100%"]
