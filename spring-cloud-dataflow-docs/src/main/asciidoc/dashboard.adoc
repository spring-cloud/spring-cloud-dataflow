...
* *Apps*: The Apps tab lists all available applications and provides the control to register/unregister them.
* *Runtime*: The Runtime tab provides the Data Flow cluster view with the list of all running applications.
* *Streams*: The Streams tab lets you list, create, deploy, and destroy Stream Definitions.
...
The Apps section of the dashboard lists all the available applications and provides the control to register and unregister them (if applicable).
...
The bottom of the page has quick links to the property files for common groups of stream apps and task apps.
If those meet your needs, select your appropriate variant (Rabbit, Kafka, Docker, and so on) and click the Import action on those lines to immediately import all those applications.
...
The Runtime section of the Dashboard application shows the Spring Cloud Data Flow cluster view with the list of all running applications.
...
Thus, if the underlying definition has been deleted, "`deleted`" appears in the Status column.
...
// TODO To get to the Job Execution Details page, what does one click?
...
The Job Execution Details page provides information about an individual job.
...
You can further drill into the details of each step's execution by clicking the magnifying glass.
