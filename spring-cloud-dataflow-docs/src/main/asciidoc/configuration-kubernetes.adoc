...
The following example shows one possible configuration, which enables RabbitMQ, and MySQL as well as basic security settings for the server:
...
The preceding example assumes that RabbitMQ is deployed with `rabbitmq` as the service name. For MySQL, it assumes that the service name is `mysql`. Kubernetes publishes the host and port values of these services as environment variables that we can use when configuring the apps we deploy.
