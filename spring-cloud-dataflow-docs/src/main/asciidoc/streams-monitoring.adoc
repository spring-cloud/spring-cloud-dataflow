...
If the Data Flow server is started with the `spring.cloud.dataflow.grafana-info.url` property pointing to your Grafana URL, then the Grafana feature is enabled and SCDF UI will provide you with handy Grafana-buttons that can open particular dashboard for given stream, application or application instance. Following screenshot illustrate those buttons:

image::{dataflow-asciidoc}/images/grafana-scdf-ui-buttons-streams.png[SCDF UI Grafana Buttions - Streams, scaledwidth="70%"]

image::{dataflow-asciidoc}/images/grafana-scdf-ui-buttons-apps.png[SCDF UI Grafana Buttions - Appliations, scaledwidth="70%"]
