...
This is so that the task executions recorded by the task application can be read by Spring Cloud Data Flow.
...
or <<datasource-task-launcher-sink, Task Launcher Sink>> apply to the composed task runner.
...
https://projects.spring.io/spring-batch/[Spring Batch] jobs, must make sure that
