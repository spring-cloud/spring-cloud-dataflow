...
While Spring Cloud Task does provide a number of out-of-the-box applications (at https://github.com/spring-cloud-task-app-starters[spring-cloud-task-app-starters]), most task applications require custom developmet.  To create a custom task application:
...
Then you can use the `app import` command and provide the location of the properties file by using the  `--uri` switch, as follows:
...
This cause the properties, `prop1=value1` and `prop2=value2`, to be passed to all the launched applications.
...
To create a transitional conector:
...
Setting of `triggertask.environment-properties` lets all the task executions can be collected in the same H2 database used in the local version of the Data Flow Server.
