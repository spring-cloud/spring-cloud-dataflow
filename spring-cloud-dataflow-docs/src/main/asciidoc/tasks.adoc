...
command line or the Task Launcher sink.
In either of these cases you must configure the Composed Task Runner to use the
same datasource that Spring Cloud Data Flow is using. In the command line case
make sure the Command Line Runner is configured via command line args or properties
to use the correct datasource.  In the Task Launcher sink case, be sure to set the
datasource properties via the `TaskLaunchRequest`.  This is because, the
