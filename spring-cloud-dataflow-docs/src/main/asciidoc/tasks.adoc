...
```
dataflow:> app register --name timestamp --type task --uri maven://org.springframework.cloud.task.app:timestamp-task:1.2.0.RELEASE
...
applications that comprises our composed task as well as the composed task
...
the other tasks in sequential order.
...
=== Configuring Composed Task in Spring Cloud Data Flow
...
Out of the box this application is not registered with Spring Cloud Data Flow
by default.  So to create composed tasks we must first register the Composed
Task Runner as an application with Spring Cloud Data Flow as follows:
```
app register --name composed-task-runner --type task --uri maven://org.springframework.cloud.task.app:composedtaskrunner-task:1.2.0.RELEASE
```
You can also configure Spring Cloud Data Flow to use a different task definition
name for the composed task runner.  This can be done by setting the following
property `spring.cloud.dataflow.task.composedTaskRunnerName` to the name
of your choice.  You can then register the composed task runner application with
the name you set using the property.
