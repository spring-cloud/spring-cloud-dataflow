...
```
task create CTR-AAA --definition "AAA"
task create CTR-BBB --definition "BBB"
```
...
```
stream create ctr-stream --definition "time --fixed-delay=30 | tasklaunchrequest-transform --uri=maven://org.springframework.cloud.task.app:composedtaskrunner-task:<current release> --command-line-arguments='--graph=taskfun-AAA&&taskfun-BBB --increment-instance-enabled=true --spring.datasource.url=...' | task-launcher-local"
```
...
* *spring.datasource.** - establishes the datasource that is used by Spring Cloud
Data Flow.  This is so that the `ComposedTaskRunner` can update the state of
its execution.
