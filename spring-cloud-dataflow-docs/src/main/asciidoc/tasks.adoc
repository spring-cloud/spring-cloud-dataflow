...
The general lifecycle of a Task is:

. Register Applications.
. Create Task Definition.
. Launch Task.

You can use the Local Data Flow Server to launch tasks to your local machine, the Data Flow Server for Cloud Foundry to launch tasks to a single org and space on Cloud Foundry.
Similarly, you can use Data Flow Server for Kuberenetes to launch a task to a single namespace on a Kubernetes cluster.
...
NOTE: Launching Spring Cloud Task applications can not be delegated to Skipper at this time.  You must use the Data Flow Server option to launch tasks.

[[spring-cloud-dataflow-stream-intro-dsl]]
=== Launching a task

A task is defined by specifying the Task application you wish to execute along with any properties that you want to remain static for the definition.
Each Spring Cloud Task application is registered under a simple name.
The registration process specifies where the application can be obtained (for example, in a Maven Repository or a Docker registry).  You can find out more information on how to register Spring Cloud Task applications in this <<spring-cloud-dataflow-register-task-apps,section>>.

As a simple example, consider printing a timestamp to the console.
The Task definition to use the timestamp application is simply:

`timestamp`

Task definitions can be created by using the shell's `task create` command, as shown in the following example:

`dataflow:> task create --name printTimeStamp --definition "timestamp"`

The Task definition is passed in to the `--definition` command option.
