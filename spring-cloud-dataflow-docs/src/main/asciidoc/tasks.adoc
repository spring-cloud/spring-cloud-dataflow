...
The `<platform-type>` refers to one of the currently supported deployers: `local` or `kubernetes` For `cloudfoundry`, the property is `spring.cloud.dataflow.task.platform.<platform-type>.accounts[<account-name>].deployment.maximumConcurrentTasks` (add `deployment` to the path).
