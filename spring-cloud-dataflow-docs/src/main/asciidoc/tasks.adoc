...
In some cases a task that is executing on a platform may not stop because of a problem on the platform or the software itself.
...
----
In the example above we see that the user has requested that task execution id 5 to be stopped.
In these cases the platform will stop the execution.  When we view the result for the task execution we will see that the task execution completed with a 0 exit code.
