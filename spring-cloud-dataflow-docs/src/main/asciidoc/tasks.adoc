...
```
The remote maven repositories need to be configured explicitly unless the task artifacts being launched are in the local repository.
For Example:
```
stream create task-stream2 --definition "http --port=9000 | tasklaunchrequest-transform --uri=maven://org.springframework.cloud.task.app:timestamp-task:jar:1.2.0.RC1 | task-launcher-local --maven.remote-repositories.repo1.url=http://repo.spring.io/libs-snapshot"
```
In the example above we see that we have configured `maven.remote-repositories.repo1.url` to use the `http://repo.spring.io/libs-snapshot` repository.
...
An example of the triggertask
would be to launch the timestamp task once every 5 seconds, the stream to implement this would look like:

```
stream create foo --definition "triggertask --triggertask.uri=maven://org.springframework.cloud.task.app:timestamp-task:jar:1.2.0.RELEASE --trigger.fixed-delay=5 | task-launcher-local --maven.remote-repositories.repo1.url=http://repo.spring.io/libs-snapshot" --deploy
```

=== TaskLaunchRequest-transform

Another option to start a task using the `task-launcher` would be to create a stream using the
https://github.com/spring-cloud/spring-cloud-stream-app-starters/tree/master/processor/spring-cloud-starter-stream-processor-tasklaunchrequest-transform[Tasklaunchrequest-transform] processor to translate a message payload to a TaskLaunchRequest.
...
An example of using the tasklaunchrequest-transform is as follows:

```
stream create task-stream --definition "http --port=9000 | tasklaunchrequest-transform --uri=maven://org.springframework.cloud.task.app:timestamp-task:jar:1.2.0.RELEASE | task-launcher-local --maven.remote-repositories.repo1.url=http://repo.spring.io/libs-snapshot"
```
