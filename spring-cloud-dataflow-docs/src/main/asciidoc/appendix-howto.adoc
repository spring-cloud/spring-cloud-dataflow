...
If the applications exist on another remote repository, besides the pre-configured ones, that remote repository must be configured explicitly.
...
You can also use the set the properties as individual environment variables:
