...
==== Remote Debugging with Local Deployer

When using Local Deployer you may want to benefit from the ability to debug your individual applications. This could be accomplished using remote debugging feature of the JVM.
While generally a straight forward process, Local Deployer simplifies it even further by allowing you to manage it with deployment properties.
