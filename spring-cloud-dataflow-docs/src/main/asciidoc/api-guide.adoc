...
under the covers to make a proxy Http request to the
Metrics Collector. Therefore, the endpoint will not generate an error in case the
...
The metrics data returned in the example is based on the example stream definition created
in chapter <<configuration-monitoring-deployed-applications>> `time | log`.
