...
The REST endpoint `/about` provides information on the features enabled/disabled.
...
Migration scripts for specific database types can be found at  `https://github.com/spring-cloud/spring-cloud-dataflow/spring-cloud-dataflow-server-core/src/main/resources/migration/1.3.x`
...
that is returned for each of the dependencies is configurable via the following
properties:
...
* spring.cloud.dataflow.version-info.spring-cloud-dataflow-shell.checksum-sha1 - the
checksum1 value that will be returned with the shell dependency info.
* spring.cloud.dataflow.version-info.spring-cloud-dataflow-shell.checksum-sha256 -
the checksum256 value that will be returned with the shell dependency info.
...
is not specified then SCDF will use contents of the file specified at this URL for the checksum.
...
By default checksum values will not be displayed for the shell dependency if
you need this feature enabled then set the
