...
Wagon based http transport is enable by setting `maven.use-wagon` property
to `true` and then _preemptive_ authentication authentication can be enabled
per remote repository. Configuration loosely follows similar patterns found
in https://maven.apache.org/guides/mini/guide-http-settings.html[HttpClient HTTP Wagon].
Documentation in Maven's own site is slightly misleading and missing most
of the possible config options.
