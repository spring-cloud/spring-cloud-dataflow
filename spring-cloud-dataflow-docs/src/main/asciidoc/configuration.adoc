...
By default, the logging configuration uses the following settings for `LOG_CONFIG`, `LOG_FILE` and `LOG_PATH`.

----
logging:
  config: classpath:logback-scdf-local.xml
  path: scdf-logs
  file: spring-cloud-dataflow-server-local
----

The logback configuration `logback-scdf-local.xml` uses the following `RollingPolicy` configuration:

----

<fileNamePattern>${LOG_PATH}/${LOG_FILE}.${LOG_FILE_ROLLING_FILE_NAME_PATTERN:-%d{yyyy-MM-dd}}.%i.gz</fileNamePattern>
...
If you want to override any of the properties `LOG_FILE_MAX_SIZE`, `LOG_FILE_MAX_HISTORY` and `LOG_FILE_TOTAL_SIZE_CAP`, please set them as system properties.
