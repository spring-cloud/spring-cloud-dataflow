...
* In `IntelliJ`, install the plugin `Eclipse Code Formatter`.  You can find it doing a plugin search within `IntelliJ`.  Then navigate `File -> Other Settings -> Default Settings` and select the Eclipse Code Formatter.  Select the the `eclipse-code-formatter.xml` file for the field `Eclipse Java Formatter config file` and the file `eclipse.importorder` for the field `Import order`.
