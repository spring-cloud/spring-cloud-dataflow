...
To test the latest dataflow server, make sure you have a local spring cloud data flow docker images is build:

----
./mvnw clean install -DskipTests
./mvnw -pl spring-cloud-dataflow-server docker:build

docker images | grep dataflow
springcloud/spring-cloud-dataflow-server               2.4.1.BUILD-SNAPSHOT   9b28616761d3        About a minute ago   344MB
springcloud/spring-cloud-dataflow-server               latest                 9b28616761d3        About a minute ago   344MB
....
----
...
./mvnw clean test-compile failsafe:integration-test -pl spring-cloud-dataflow-server -Pfailsafe
   ...
   -Dtest.docker.compose.dataflow.version=2.4.1.BUILD-SNAPSHOT
   -Dtest.docker.compose.skipper.version=2.3.1.BUILD-SNAPSHOT
----
...
Use `-Dtest.docker.compose.pullonstartup=false` parameter to make sure that the local docker images are used with precedence.
