...
public class RuntimeApplicationHelper {
    ...
    public void httpPost(String streamName, String appName, String message) {
        httpPost(streamName, appName, message, null);
    }
    ...
    public void httpPost(final String streamName, final String appName, final String message, final HttpHeaders headers) {
        AppInstanceStatusResource instance = null;
        AppStatusResource app = null;
        StreamStatusResource streamStatusResource = this.dataFlowTemplate.runtimeOperations().streamStatus(streamName).getContent().stream().filter(stream -> stream.getName().equals(streamName)).findFirst().orElse(null);
        if (streamStatusResource != null) {
            app = streamStatusResource.getApplications().getContent().stream().filter(appStatusResource -> appStatusResource.getName().equals(appName)).findFirst().orElse(null);
            if (app != null) {
                instance = app.getInstances().getContent().stream().findFirst().orElse(null);
            }
        }
        ...
        if (headers != null) {
            ...
            this.dataFlowTemplate.runtimeOperations().postToUrl(app.getDeploymentId(), instance.getInstanceId(), message);
        }
    }
