...
public class DataflowCluster implements Startable {
	...
	public void startIdentityProvider(String id) {
		...
		oauthContainer.withLogConsumer(new Slf4jLogConsumer(logger).withPrefix("oauth"));
	...
	private SkipperContainer<?> buildSkipperContainer(
		...
		if (oauthContainer != null) {
			if(oauthContainer.getDockerImageName().contains(TagNames.UAA)) {
		...
		skipperContainer.withLogConsumer(new Slf4jLogConsumer(logger).withPrefix("skipper"));
	...
	private DataflowContainer<?> buildDataflowContainer(
		...
		dataflowContainer.withEnv("SPRING_CLOUD_SKIPPER_CLIENT_SERVER_URI", String.format("http://%s:%s/api", "skipper", SKIPPER_PORT));
		...
		if (oauthContainer != null) {
			if(oauthContainer.getDockerImageName().contains(TagNames.UAA)) {
				...
				// dataflowContainer.withEnv("LOGGING_LEVEL_ROOT", "DEBUG");
				dataflowContainer.withEnv("LOGGING_LEVEL_ORG_SPRINGFRAMEWORK_SECURITY", "DEBUG");
				...
				// dataflowContainer.withEnv("SPRING_SECURITY_OAUTH2_CLIENT_REGISTRATION_KEYCLOAK_CLIENTAUTHENTICATIONMETHOD", "post");
		...
		dataflowContainer.withLogConsumer(new Slf4jLogConsumer(logger).withPrefix("dataflow"));
